...
class ClosestPair:
    ...
    def compute(self, file_data):
        ...
        return self.closest  # this is what we really care about in terms of return values
    ...
    def dc_cpp(self, x_list, y_list):
        ...
        for i in range(len(x_list)):
            if middlePoint[0] - left_right_min <= x_list[i][0] <= middlePoint[0] + left_right_min:
                runway_points.append(x_list[i])

        runway_points.sort(key=lambda n: n[1])

        runway_min = self.runway(runway_points, left_right_min)
    ...
    def runway(self, points, delta):
        min_dist = delta

        for i in range(len(points)):
            j = i + 1
            while j < len(points) and (points[j][1] - points[i][1]) < delta:
                min_dist = self.find_dist(points[i], points[j])
                j += 1
        return min_dist
